...
func getJobsForMakeBatch(batch *models.Batch, c *config.Config) []*models.Job {
	...
	// Finally, the last jobs copy the essential files to the live path so we can
	// ingest them into staging
...
func QueueBatchGoLive(batch *models.Batch, c *config.Config) error {
	...
	jobs = append(jobs, batch.BuildJob(models.JobTypeBatchAction, makeActionArgs("moved to archive location")))
