...
func main() {
	...
	for _, issue := range issues {
		var finalDest = filepath.Join(opts.Dest, issue.HumanName)
		err = fileutil.CopyDirectory(issue.Location, finalDest)
		// We halt on any error because this can mean things like filesystem
		// problems / network disk disconnects, etc.  Better to have to rerun the
		// script than wade through dozens of errors to determine if any are
		// different.
		if err != nil {
		...
		if err != nil {
			...
			// In all cases, gtfo because something messed up is going on
			break
		}
