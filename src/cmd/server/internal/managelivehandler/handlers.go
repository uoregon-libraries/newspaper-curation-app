...
func allTitles() (titles schema.TitleList, err error) {
	...
	if err != nil {
		return nil, fmt.Errorf("looking up titles for live handler form: %w", err)
	}
...
func getJSONIssues(_ *responder.Responder) (*jsonResponse, error) {
	/*
		var moc = resp.Request.FormValue("moc")
		var lccn = resp.Request.FormValue("lccn")
		var dt = resp.Request.FormValue("date")
		var url = resp.Request.FormValue("url")
	*/

	var err error
	var response = &jsonResponse{Code: http.StatusOK}
	var finder = models.Issues().InWorkflowStep(schema.WSInProduction)
